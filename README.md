...
| **Tasks & preemptive scheduler** | Round-robin over a circular run queue, driven only by the timer — nothing is cooperative. The trap stub already saves every register, so a context switch is just *returning a different frame than the one you were handed*, and the same four lines of assembly that resume a task also start a brand-new one. A new task has never trapped, so its first frame is **fabricated by hand** to look like one the hardware would have written. Deciding to switch and actually switching are deliberately separate, so the kernel is *preempted at trap exit* rather than preemptible. |
